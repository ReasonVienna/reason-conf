/**
   This file was copied from the original basicscroll
   project.

   https://github.com/electerious/basicScroll/blob/master/src/scripts/main.js

   We need this file, since the webpack config does not work with ES6 modules &
   import statements.

   ATTENTION:
   We also changed the way the animation loop is started...
**/
 ...
 */
const relativeToAbsoluteValue = function(
  value,
  elem,
  ...
  if (elemAnchor === "middle")
    y += elemSize.top + scrollTop + elemSize.height / 2;
...
const validate = function(data = {}) {
  ...
  if (typeof data.inside !== "function")
    throw new Error("Property `inside` must be a function");
  if (typeof data.outside !== "function")
    throw new Error("Property `outside` must be a function");

  if (data.elem == null) {
    if (isAbsoluteValue(data.from) === false)
      throw new Error(
        "Property `from` must be a absolute value when no `elem` has been provided"
      );
    if (isAbsoluteValue(data.to) === false)
      throw new Error(
        "Property `to` must be a absolute value when no `elem` has been provided"
      );
  } else {
    if (isRelativeValue(data.from) === true)
      data.from = relativeToAbsoluteValue(data.from, data.elem);
    if (isRelativeValue(data.to) === true)
      data.to = relativeToAbsoluteValue(data.to, data.elem);
  }
  ...
  Object.keys(data.props).forEach(key => {
    ...
    if (isAbsoluteValue(prop.from) === false)
      throw new Error("Property `from` of prop must be a absolute value");
    if (isAbsoluteValue(prop.to) === false)
      throw new Error("Property `from` of prop must be a absolute value");

    prop.from = parseAbsoluteValue(prop.from);
    prop.to = parseAbsoluteValue(prop.to);

    if (typeof prop.timing === "string" && eases[prop.timing] == null)
    ...
    if (prop.direct === true && data.elem == null)
...
const create = function(data) {
  /**
     LOCAL MODIFICATIONS FOR REASON-CONF!
  **/
  // Start to loop on first create
  if (!isLooping) {
    loop();
    isLooping = true;
  }
